"""Format-fix adapter: runs ruff format on an allowlisted path."""
...
_ADAPTER_NAME = "format_fix"


class FormatFixAdapter:
    """Runs ruff format on a target Python file."""
    ...
    def execute(self, inp: AdapterInput) -> AdapterOutput:
        ...
        if inp.dry_run:
            return AdapterOutput(
                outcome="dry_run",
                stdout_summary=f"[dry_run] would format {inp.target_ref}",
                dry_run=True,
                started_at=started,
                finished_at=utcnow_iso(),
            )
        ...
        try:
            result = subprocess.run(
                ["python", "-m", "ruff", "format", full_path],
                capture_output=True,
                text=True,
                timeout=30,
            )
